...
class BRnet_decoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True):
        super(BRnet_decoder, self).__init__()
        ...
        for s in self.scales:
            self.convs[("dispconv", s)] = Conv3x3(self.num_ch_dec[s] if s != 3 else self.num_ch_dec[s], self.num_output_channels)
