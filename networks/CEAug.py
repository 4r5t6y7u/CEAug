...
def _hrnet(arch, pretrained, progress, **kwargs):
    ...
    if pretrained:
        if arch == 'hrnet64':
            ...
            pth_path = "./networks/HRNet_W18_C_cosinelr_cutmix_300epoch.pth.tar"
...
def hrnet18_encoder(pretrained=True, progress=True, **kwargs):
    r"""HRNet-18 model
    """
...
def hrnet48_test(pretrained=True, progress=True, **kwargs):
    r"""HRNet-48 model
    """
...
class CEAug_decoder(nn.Module):
    def __init__(self, num_ch_enc, num_output_channels=1, use_skips=True):
        ...
        self.num_ch_dec = np.array([18, 18, 18, 36, 72])
        ...
        ca_list = 0
        for ca_i in self.num_ch_dec:
            self.ca[str(ca_list)] = ChannelAttention_reduction_8(ca_i)
            ca_list +=1
