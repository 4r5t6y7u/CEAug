...
class ConvModule(nn.Module):
    """A simple wrapper for Conv2d + (optional) BatchNorm + Activation"""
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0,
                 conv_cfg=None, norm_cfg=None, act_cfg=dict(type='ReLU'), inplace=True):
        super(ConvModule, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=False)

        # If norm_cfg is provided, add BatchNorm
        self.norm = None
        if norm_cfg and norm_cfg['type'] == 'BN':
            self.norm = nn.BatchNorm2d(out_channels)

        # If act_cfg is provided, add Activation
        self.activate = None
        if act_cfg and act_cfg['type'] == 'ReLU':
