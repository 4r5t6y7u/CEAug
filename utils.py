...
def cosine_schedule_with_warmup(optimizer: Optimizer,
    ...
    def lr_lambda(current_step):
        ...
        progress = float(current_step - num_warmup_steps) / float( max(1, num_training_steps - num_warmup_steps + 1))
        return max(0.0, 0.5 * (1.0 + np.cos(np.pi * float(num_cycles) * 2.0 * progress))*(1-eta_min) + eta_min)
