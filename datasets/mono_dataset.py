...
class MonoDataset(data.Dataset):
    ...
    def preprocess(self, inputs, color_aug):
        ...
        """
        if self.CEAug:
            rd_ratio = random.uniform(0.25, 0.75)
            rd_ratio = round(rd_ratio, 2)
            inputs[("rd_ratio")] = rd_ratio
        ...
        for k in list(inputs):
            ...
            if "color" in k:
                ...
                for i in range(self.num_scales):
                    ...
                    if self.CEAug:
                        if i == 0 and im == 0:
                            inputs[(n+"_top", im, i)] = self.resize[i](inputs[(n, im, i - 1)].crop((0, 0, w, h*rd_ratio)))
                            inputs[(n+"_bottom", im, i)] = self.resize[i](inputs[(n, im, i - 1)].crop((0, h*rd_ratio, w, h)))

                            inputs[(n+"_left", im, i)] = self.resize[i](inputs[(n, im, i - 1)].crop((0, 0, w*rd_ratio, h)))
                            inputs[(n+"_right", im, i)] = self.resize[i](inputs[(n, im, i - 1)].crop((w*rd_ratio, 0, w, h)))
